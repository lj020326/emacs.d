...
;; Version: 2.09?  RMS has an obnoxious tendency to remove version
;; numbers from packages, and he did in this case. (In any case,
;; the primary source of this package is now within Emacs, so this
;; version number is passing irrelevant.)
...
(defgroup desktop nil
  "Save partial status of Emacs when you exit."
  :group 'frames)
...
`must' (values must be quoted), or nil (value may not be quoted)."
  (cond
   ((or (numberp val) (characterp val) (null val) (eq t val))
...
(defun desktop-remove ()
  ...
  (interactive)
  (if desktop-dirname
      (let ((filename (expand-file-name desktop-basefilename desktop-dirname)))
